...
这里要注意一个概念边界：云原生不等于简单的“即插即用”。“即插即用”更像是组件化带来的工程效果之一，而不是云原生数据库的定义。

更准确地说，云原生数据库通常强调：

- 存算分离：计算节点可以扩缩容，数据不强绑定某一台机器的本地盘。
- 存储抽象：底层可以适配 page store、共享存储、分布式文件系统、对象存储或自研存储服务。
- 弹性调度：节点可以快速拉起、下线、扩容和迁移。
- 多租户隔离：多个租户共享基础设施，但数据、资源和故障边界需要隔离。
- 高可用和自动恢复：节点故障后可以切换、重放和恢复。
- 自动化运维：部署、升级、备份、恢复、监控和扩缩容尽量平台化。
- 组件化边界：计算、存储、日志、元数据、复制和调度之间有清晰接口。

所以如果说 D-Store “像一个即插即用的引擎”，这个说法可以作为直觉入口，但需要更精确地表达为：

```text
D-Store 更像一个组件化的独立存储内核。
它具备被接入 SQL 层、适配 page store/VFS、多租户和云原生存储后端的能力。
但是否真正做到即插即用，还取决于它和 SQL 层、事务、WAL、catalog、恢复协议之间的集成边界是否稳定。
```
...
也就是说，D-Store 不是简单回答“一张表的数据怎么组织”，而是在回答更大一层的问题：

```text
数据库的整个存储层能不能被抽成一个独立组件？
能不能适配 page store 或其他云上存储后端？
能不能支持租户隔离？
能不能支持存算分离？
能不能在分布式/云环境里完成恢复、切换和多写协同？
```

因此，D-Store 的“独立编译、独立测试、page store/VFS 抽象”更像是为了实现云原生存储内核能力而做的组件化设计，而不是简单意义上的插件式表引擎。
