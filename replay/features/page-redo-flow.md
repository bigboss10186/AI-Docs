...
为了避免每个标题看起来都像独立概念，读这篇时要一直跟踪同一个对象：**当前正在被 startup process 处理的一条 WAL record**。

这条 record 在不同阶段会不断“长出”新的上下文：

| 阶段 | 当前手里有什么 | 下一步要解决什么 |
| --- | --- | --- |
| recovery 启动 | `pg_control`、checkpoint record、`checkpoint.redo` | 从 WAL 的哪个位置开始读 |
| 读取 record | `XLogReaderState`、`XLogRecord`、`EndRecPtr` | 这条 record 属于哪个 rmgr |
| rmgr 分发 | `RmgrId`、`xl_info`、main data | 由哪个 redo 函数解释 payload |
| block 定位 | block reference、relation/fork/block | 目标页面在哪里 |
| 准备 buffer | relation 文件、目标 block、buffer lock | 页面是否存在，是否要预扩或初始化 |
| FPI/LSN 判断 | full page image、page LSN、record LSN | 是恢复整页、跳过，还是做逻辑 redo |
| 页面修改 | heap/btree/smgr 的具体 redo 逻辑 | 如何按 WAL 指定结果修改 page |
| 收尾 | page LSN、dirty flag、buffer pin/lock | 让后续重复 redo 可跳过，并把脏页交给 buffer manager |

如果用 heap insert 作为贯穿例子，完整故事是：

```text
checkpoint.redo 告诉 recovery 从哪里开始
  -> startup process 读到一条 RM_HEAP_ID record
  -> heap_redo() 发现它是 XLOG_HEAP_INSERT
  -> heap_xlog_insert() 从 block reference 取出 relation/fork/block
  -> xlogutils 把目标 block 变成 locked buffer
  -> 如果有 FPI，先恢复整页
  -> 如果没有 FPI，用 page LSN 判断是否需要重放
  -> 需要重放时，按 WAL 指定 offset 插入 tuple
  -> 设置 page LSN，标脏，释放 buffer
```

后面每个章节都在解释这条链路上的一个关口。读的时候不要只问“这个概念是什么”，更要问“它解决了上一阶段留下的哪个问题，并给下一阶段提供了什么输入”。

还要把三条状态线分清楚：

| 状态线 | 它在描述什么 | 和 redo 的关系 |
| --- | --- | --- |
| WAL 位置线 | 当前读到哪个 record，已经成功 replay 到哪个 `EndRecPtr` | 由 `ReadRecord()`、`ApplyWalRecord()`、`lastReplayedEndRecPtr` 推进 |
| buffer page 线 | 目标 page 是否在 shared buffer 中，是否已被当前 record 修改 | 由 `XLogReadBufferForRedo()`、具体 redo、`PageSetLSN()` 管理 |
| 磁盘文件线 | relation 文件是否存在，block 是否存在，dirty page 是否已经写回 | 由 smgr、buffer manager、checkpoint/restartpoint、eviction 等路径管理 |

这三条线不是同步推进的。redo 可以先把 page 修改到 shared buffer 并标脏，不要求这条 record 修改过的 page 立刻落盘；如果之后再次崩溃，仍然依赖 WAL、FPI 和 page LSN 重新恢复。这一点是理解 PostgreSQL page redo 的关键。
...
这一节是整个流程的入口。它不关心某一条 heap 或 btree record 怎么改 page，只负责回答一个问题：**startup process 应该从 WAL 的哪个 LSN 开始读第一条需要 redo 的 record**。一旦 `checkpoint.redo` 被确定，后续章节才有“当前 record”可以读取、解析和应用。
...
上一节确定了“从哪里开始读”，这一节解决“读出来的是什么”。注意这里还没有修改任何 page。`ReadRecord()` 的任务是把 WAL 字节流变成 redo 函数能消费的结构化对象：header 告诉 recovery 这条 record 的基本身份，main data 和 block reference 告诉后面的 rmgr redo 应该如何理解它。
...
上一节拿到的是一条通用 WAL record。通用 reader 不知道 heap insert 的 tuple data 怎么解释，也不知道 btree split 的 high key 怎么恢复。因此这里必须先分发：`RmgrId` 把“通用 record”交给“懂这个 payload 的模块”。从这一刻开始，流程进入 heap、btree、smgr、xact 等具体子系统。
...
上一节确定了由哪个 rmgr redo 函数解释 record；现在 redo 函数要先找到它要修改的页面。这个定位不是通过表名、SQL 条件或执行计划完成的，而是直接使用 WAL block reference 里的物理身份：relation、fork、block。后面所有 FPI、page LSN、PageAddItem、btree link 修复，都必须先落到这个物理 page 上。
...
上一节只是在 WAL 里解析出了“我要访问哪个 relation/fork/block”。但 WAL 里有物理地址，不代表磁盘上此刻已经有对应文件。恢复时内存是重新初始化的，磁盘状态可能停在崩溃前某个不完整瞬间，所以 xlogutils/smgr 必须把“WAL 指向的页面”转换成“当前能被 buffer manager 管理的页面”。这就是 relation 文件存在性检查的作用。
...
这一节接着上一节往下走：文件存在还不够，目标 block 也必须存在。比如 WAL 说要修改 block 100，但崩溃后的 relation 文件只有 80 个 block，redo 不能重新选择一个别的 block，也不能直接跳过。它必须把 relation 扩展到 WAL 指定的物理位置，然后再由后续逻辑恢复该 block 的内容。
...
到这里，流程已经知道目标 relation/fork/block，并且已经处理了文件不存在或文件太小的问题。下一步是把目标 block 变成一个带 pin、带 lock 的 buffer。只有拿到这个 buffer，后面的 FPI 恢复、page LSN 判断、heap/btree 页面修改才有地方执行。
...
上一节已经拿到了目标 buffer，但还不能马上执行逻辑 redo。原因是磁盘上的旧页面可能不可信，特别是 torn page 场景。FPI 是 redo 的第一道页面内容分支：如果 WAL record 为这个 block 提供了整页镜像，并且需要应用，recovery 可以直接把整页恢复成 WAL 中的版本，而不是依赖磁盘旧页再做增量修改。
...
如果没有走 FPI 整页恢复，redo 就要面对另一个问题：这个页面是不是已经包含当前 WAL record 的结果？崩溃前有些 page 可能已经刷盘，有些没有。page LSN 判断就是把“是否需要重放”从猜测变成页面自己的状态判断。只有判断结果是需要 redo，后面的 heap insert、heap update、btree split 才真正改 page。
...
前面几节讲的是所有 rmgr redo 都要经过的公共关口：找 record、分发、定位 block、准备 buffer、处理 FPI/page LSN。从这里开始进入具体页面修改。heap insert 是最适合入门的例子，因为它能直接体现 PostgreSQL redo 的核心原则：**正常执行时已经决定了 tuple 放在哪个 block、哪个 offset；恢复时只负责把这个结果修回去**。
...
理解 heap insert 后，再看 update 会自然一些。insert 只是在一个目标 offset 放入新 tuple；update 则要同时维护“旧版本如何失效”和“新版本放在哪里”。所以 update redo 的难点不是重新执行 UPDATE 语句，而是根据 WAL 恢复两个物理结果：旧 tuple header 被改成已更新状态，新 tuple 被插入到指定位置。
...
delete redo 和 update 的 old tuple 部分很像：它也不是把 tuple 从 page 上抹掉，而是恢复 MVCC 所需的 header 状态。读到这一节时要把“删除”理解成“给指定 offset 的 tuple 写入删除事务信息”，不是“释放 line pointer 或压缩 page”。真正物理清理属于 pruning/vacuum 的后续工作。
...
heap redo 修改的是表数据页，btree redo 修改的是索引页。它们都遵循同一条公共 redo 骨架：定位 block、读 buffer、page LSN 判断、修改 page、设置 LSN、标脏。但 btree 页面的正确性不只取决于 item 是否插进去，还取决于 high key、rightlink、page opaque 等搜索结构是否一致。
...
btree insert 是单页插入的入口，btree split 则是“单页放不下”后的结构调整。这里要把 split 看成一组页面状态变更，而不是一条简单插入：left page、right page、sibling、parent 之间必须形成一个可搜索的中间状态。WAL redo 的目标是恢复这个已经决定好的页面拓扑。
...
这一节容易被误读成附属知识。它实际回答的是：一条 heap WAL record 除了修改 heap page，为什么还可能影响辅助 fork？heap page 是主数据，但 visibility map 和 FSM 会影响 index-only scan、vacuum、页面选择等行为。redo 必须保证影响正确性的辅助状态不会比 heap page 更“乐观”。
...
这是所有页面级 redo 的共同收尾。前面 heap insert、heap update、heap delete、btree insert、btree split 的具体动作不同，但只要真的改了 page，最后都要把这次修改变成 buffer manager 和未来 recovery 能理解的状态：page LSN 表示“我已经应用到哪里”，dirty 标记表示“这个内存页未来要写回磁盘”，释放 buffer 表示“这个页面修改窗口结束”。
...
这一节把前面分散的概念按真实顺序串起来。读的时候可以把它当成“从源码入口一路跟到 page 修改”的检查清单：每一步都能在前面的章节找到对应解释。
...
| heap insert redo 步骤 | 对应章节 | 关键点 |
| --- | --- | --- |
| 从 checkpoint.redo 开始读 WAL | 2 | 先确定恢复起点，不从 WAL 开头扫 |
| 读到 heap insert record | 3 | reader 只负责解析通用 record |
| 分发到 `heap_redo()` | 4 | `RmgrId` 决定由 heap 解释 payload |
| 取 relation/fork/block/offnum | 5 | WAL 指定物理目标，不重新选 page |
| 检查文件和 block | 6、7 | 目标文件可能不存在或大小不足 |
| 取得 locked buffer | 8 | 后续修改都发生在 shared buffer page 上 |
| FPI 或 page LSN 判断 | 9、10 | 决定恢复整页、跳过、还是逻辑 redo |
| 插入 tuple 并清 VM | 11、16 | 恢复 heap page，同时修正可见性辅助状态 |
| 设置 LSN、标脏、释放 | 17 | redo 收尾，不要求同步写盘 |
...
这个例子用于说明：并不是所有 WAL record 都只改一个 block。btree split 的复杂度来自“多个页面之间的结构关系”。如果前面 heap insert 是单页 redo 的入门例子，那么 btree split 就是多页 redo 和中间一致性的代表。
...
| btree split redo 步骤 | 对应章节 | 关键点 |
| --- | --- | --- |
| 分发到 `btree_redo()` | 4 | `RM_BTREE_ID` 决定由 btree 解释 payload |
| 解析 left/right/sibling block | 5 | 一条 record 可以有多个 block reference |
| 初始化 right page | 8、15 | 新 page 通常由 WAL 信息重建 |
| 重建 left page | 10、15 | 如果 page LSN 较旧才需要 redo |
| 更新 sibling/parent 相关状态 | 15 | 保证页面链接关系可恢复 |
| 设置 incomplete split | 15 | 表示 parent downlink 可能还要收尾 |
| 设置 LSN、标脏、释放多个 buffer | 17 | 多个页面都要独立完成 redo 收尾 |
