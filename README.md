...
普通笔记、源码阅读和专题分析文档，建议在标题下方使用中文表格记录文档信息。这样在 Markdown 中更易读，也更符合个人知识库的阅读习惯。
...
- `概念解释`: 解释一个机制、术语或模型。
- `源码阅读`: 记录入口、调用链、数据结构和源码坐标。
- `排障复盘`: 记录问题现象、定位过程、根因和后续检查项。
- `普通笔记`: 记录暂时不属于固定模板的知识点。
- `阶段总结`: 总结一段时间的学习成果和下一步问题。
- `问题清单`: 收集尚未解决、需要继续追问的问题。
