# Source Reading Index

## 范围

源码阅读相关笔记，重点记录入口、调用链、关键结构、状态变化和源码坐标。

## 归档维度

- 入口: CLI 命令、API handler、后台 worker、测试用例、日志关键词。
- 调用链: 从入口函数到核心机制的关键路径。
- 状态: 关键结构体、枚举、全局状态和配置项。
- 证据: 源码、日志、实验、文档。
- 结论: 用自己的话解释机制。

## 相关模板

- [源码阅读模板](../templates/source-map.md)
- [概念笔记模板](../templates/concept.md)
- [问题复盘模板](../templates/debug.md)

## 已归档笔记

- [Codex Rust Workspace 分层索引](../codex/architecture/codex-rs-index.md)
- [Codex 记忆系统实现分析](../codex/memory/codex-memory-system.md)
- [Replay 源码地图](../postgres/replay/source-map.md)
- [openGauss DCF 模式下 XLog Entry 的切分、传输与落盘流程](../gaussdb/replication/dcf-xlog-transport-flow.md)
